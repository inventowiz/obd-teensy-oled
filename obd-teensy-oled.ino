...
FlexCAN_T4<CAN_DEVICE, RX_SIZE_256, TX_SIZE_16> can;
...
ESP8266_SSD1322 oled(OLED_DC, OLED_RESET, OLED_CS);
// data divisions
static const uint8_t leftLineX = (oled.width()/2)-(6*5);
static const uint8_t rightLineX = (oled.width()/2)+(6*5);
static const uint8_t leftDiv1Y = (oled.height()/4);
static const uint8_t leftDiv2Y = 2*(oled.height()/4)+2;
static const uint8_t leftDiv3X = (oled.width()/4)-16;
static const uint8_t rightDiv1Y = (oled.height()/4);
static const uint8_t rightDiv2Y = 2*(oled.height()/4)+2;
static const uint8_t rightDiv3Y = 3*(oled.height()/4)-2;
...
volatile float fuel_percent = 100*((*miles_to_e/10.0)/((*fuel_mpg_trip/5.0)*FUEL_CAP_GAL));
...
void setup() {  
  ...
  //can.setFIFOFilter(0,SUBARU_HSCAN_STEERING_SENSOR_STATUS_FRAME_ID,STD); // steering sensor status
  can.setFIFOFilterRange(0,SUBARU_HSCAN_ECU_STATUS_1_FRAME_ID,SUBARU_HSCAN_ECU_STATUS_2_FRAME_ID,STD); // ECU status 1,2
  can.setFIFOFilterRange(0,SUBARU_HSCAN_DRIVER_INPUT_FRAME_ID,SUBARU_HSCAN_BODY_STAT_1_FRAME_ID,STD); // Driver input, speed, wheel speeds, body stat 1
  can.setFIFOFilter(0,SUBARU_HSCAN_BODY_STAT_2_FRAME_ID,STD); // body stat 2
  can.enableFIFO();
  ...
  thread_pids[PID_CAN] = threads.addThread(serviceCANTask,NULL,2048); // 10ms cycle time
...
void serviceUSBSerial(){
  while(1){
    ...
    if(flag_debug_info){
      ...
      Serial.printf("Fuel consump.:\t%d mm^3/s %f mpg %f mpg/t\n",rtd.fuel_consumption_inst, rtd.fuel_mpg_inst, (*fuel_mpg_trip)/5.0);
...
void serviceCANTask(){
  while(1){
    ...
    while(can.read(can_message)){

      if(flag_debug_can){
        Serial.printf(F("Frame received! ID: %03x D:"),can_message.id);
        for(int i=0;i<can_message.len;i++){
          Serial.printf(F(" %02x"),can_message.buf[i]);
        }
        Serial.println("");
      }
      ...
      switch(can_message.id){
        case SUBARU_HSCAN_STEERING_SENSOR_STATUS_FRAME_ID:{
          ...
          subaru_hscan_steering_sensor_status_unpack(&temp,can_message.buf,can_message.len);
        }break;
        case SUBARU_HSCAN_ECU_STATUS_1_FRAME_ID:{
          // ECU Status 1
          subaru_hscan_ecu_status_1_t temp;
          subaru_hscan_ecu_status_1_unpack(&temp,can_message.buf,can_message.len);
        ...
        case SUBARU_HSCAN_ECU_STATUS_2_FRAME_ID:{
          ...
          subaru_hscan_ecu_status_2_unpack(&temp,can_message.buf,can_message.len);
        ...
        case SUBARU_HSCAN_DRIVER_INPUT_FRAME_ID:{
          ...
          subaru_hscan_driver_input_unpack(&temp,can_message.buf,can_message.len);
        ...
        case SUBARU_HSCAN_SPEED_FRAME_ID:{
          ...
          subaru_hscan_speed_unpack(&temp,can_message.buf,can_message.len);
        ...
        case SUBARU_HSCAN_WHEEL_SPEED_FRAME_ID:{
          ...
          subaru_hscan_wheel_speed_unpack(&temp,can_message.buf,can_message.len);
        ...
        case SUBARU_HSCAN_BODY_STAT_1_FRAME_ID:{
          ...
          subaru_hscan_body_stat_1_unpack(&temp,can_message.buf,can_message.len);
        ...
        case SUBARU_HSCAN_BODY_STAT_2_FRAME_ID:{
          ...
          subaru_hscan_body_stat_2_unpack(&temp,can_message.buf,can_message.len);
...
void calcLongTerms(){
  ...
  *miles_to_e = (FUEL_CAP_GAL*fuel_percent/100.0)*((*fuel_mpg_trip)/5.0)*10;
}
...
void updateDisplayTask(){
  while(1){
    ...
    // verts
    oled.drawLine(leftLineX,leftDiv1Y,leftLineX,oled.height(),WHITE);
    oled.drawLine(rightLineX,leftDiv1Y,rightLineX,oled.height(),WHITE);
    oled.drawLine(leftDiv3X,leftDiv2Y,leftDiv3X,oled.height(),WHITE);
    // horizos
    oled.drawLine(0,leftDiv1Y,leftLineX,leftDiv1Y,WHITE);
    oled.drawLine(0,leftDiv2Y,leftLineX,leftDiv2Y,WHITE);
    oled.drawLine(rightLineX,rightDiv1Y,oled.width(),rightDiv1Y,WHITE);
    oled.drawLine(rightLineX,rightDiv2Y,oled.width(),rightDiv2Y,WHITE);
    oled.drawLine(rightLineX,rightDiv3Y,oled.width(),rightDiv3Y,WHITE);
  
    // Top center display current time
    oled.setTextSize(2);
    oled.setTextColor(WHITE);
    ...
    oled.setTextSize(2);
    oled.setCursor(oled.width()-((2*12)+(6*6)),0);
    ...
    oled.printf(F("%-3.1f  %3.1f"),((float)*fuel_mpg_trip)/5.0,rtd.fuel_mpg_inst);
    oled.setCursor(rightLineX+3+(10*6),rightDiv1Y+5);
    oled.printf(F("  MPG"));
    ...
    oled.printf(F("%3.0f"),((float)*miles_to_e)/10.0);
    oled.setTextSize(1);
    oled.setCursor(11*3,leftDiv1Y+2);
    oled.printf(F(".%01d"), getDecimalTenth(((float)*miles_to_e)/10.0));
    ...
    oled.setTextSize(1);
    oled.setCursor((oled.width()/2)-2-(6*3),leftDiv1Y+3); // top left
    oled.printf(F("%3.0f"),rtd.spd_wheel_fl/MI_TO_KM);
    oled.setCursor((oled.width()/2)+2,leftDiv1Y+3); // top right
    oled.printf(F("%-3.0f"), rtd.spd_wheel_fr/MI_TO_KM);
    oled.setCursor((oled.width()/2)-2-(6*3),rightDiv3Y-8); // bottom left
    oled.printf(F("%3.0f"),rtd.spd_wheel_rl/MI_TO_KM);
    oled.setCursor((oled.width()/2)+2,rightDiv3Y-8); // bottom right
    oled.printf(F("%-3.0f"),rtd.spd_wheel_rr/MI_TO_KM);

    // display FPS
    oled.setCursor((oled.width()/2)-(6*3.5),oled.height()-7);
    oled.printf(F("%3.0f"),(double)1000/(millis()-loopTimer));
    oled.print(F(" FPS"));
